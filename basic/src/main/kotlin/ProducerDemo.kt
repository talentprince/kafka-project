            ...
            if (exception == null) {
                logger.info(
                    StringBuffer().appendLine("Topic: ${metadata.topic()}")
                        .appendLine("Partition: ${metadata.partition()}").appendLine("Offset: ${metadata.offset()}")
                        .appendLine("Timestamp: ${Instant.ofEpochMilli(metadata.timestamp()).atOffset(ZoneOffset.ofHours(8))}")
                        .toString()
                )
            } else {
